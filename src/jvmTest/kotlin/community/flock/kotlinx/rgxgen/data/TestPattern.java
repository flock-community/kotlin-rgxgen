...
import static community.flock.kotlinx.rgxgen.model.UnicodeCategory.DECIMAL_DIGIT_NUMBER;
import static community.flock.kotlinx.rgxgen.model.UnicodeCategoryConstants.BASIC_LATIN_LOWERCASE_LATIN_ALPHABET;
import static community.flock.kotlinx.rgxgen.parsing.dflt.ConstantsProvider.*;
...
public enum TestPattern implements DataInterface {
    ...
    CATEGORY_WITHIN_SQUART_BRACKETS("[a-z\\p{Nd}]{2}", new Repeat("[a-z\\p{Nd}]{2}",
                                                                  SymbolSet.ofUnicode("\\p{Decimal_Digit_Number}",
                                                                                      Stream.concat(
                                                                                                    ...
                                                                                                    DECIMAL_DIGIT_NUMBER.symbolRanges.stream())
                                                                                            .collect(Collectors.toList()),
    ...
    private static Stream<String> getSymbolStream(UnicodeCategory category) {
        return Stream.concat(
                stream(category.symbols).map(String::valueOf),
