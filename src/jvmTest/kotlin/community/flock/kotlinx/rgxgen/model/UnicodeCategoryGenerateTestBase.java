...
import static community.flock.kotlinx.rgxgen.model.UnicodeCategory.OTHER_LETTER;
import static community.flock.kotlinx.rgxgen.model.UnicodeCategory.values;
...
public class UnicodeCategoryGenerateTestBase {
    ...
    void printGenerationStatistics() {
        for (Map.Entry<UnicodeCategory, Set<Character>> entry : generatedCharacters.entrySet()) {
            ...
            System.out.println("Category " + category + " with keys " + category.keys + " character stats: ");
            ...
            double totalValueCount = category.symbols.length + category.symbolRanges.stream().mapToInt(range -> range.getTo() - range.getFrom() + 1).sum();
    ...
    public static class CategoryFinder {
        public static void main(String[] args) {
            int minChar = 4341;
            int maxChar = 4341;
            CharList characters = CharList.charList(OTHER_LETTER.symbols);
            for (int i = minChar; i <= maxChar; i++) {
                if (characters.contains((char) i)) {
                    System.out.println(i + " found in individual characters");
                }

                for (SymbolRange symbolRange : OTHER_LETTER.symbolRanges) {
