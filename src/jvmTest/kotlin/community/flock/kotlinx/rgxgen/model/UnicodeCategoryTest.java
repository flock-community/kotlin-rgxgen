...
class UnicodeCategoryTest {
    ...
    void keysAreDefinedForEachCategoryTest(UnicodeCategory unicodeCategory) {
        assertNotNull(unicodeCategory.keys);
        assertFalse(unicodeCategory.keys.isEmpty());
    }

    @ParameterizedTest
    @EnumSource(UnicodeCategory.class)
    void descriptionIsDefinedForEachCategoryTest(UnicodeCategory unicodeCategory) {
        assertNotNull(unicodeCategory.description);
    }

    @ParameterizedTest
    @EnumSource(UnicodeCategory.class)
    void isValidCategoryTest(UnicodeCategory unicodeCategory) {
        boolean rangesPresent = unicodeCategory.symbolRanges != null && !unicodeCategory.symbolRanges.isEmpty();
        boolean symbolsPresent = unicodeCategory.symbols != null && unicodeCategory.symbols.length != 0;
